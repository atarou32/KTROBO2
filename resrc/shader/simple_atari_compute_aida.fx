...
#define KTROBO_MAX_TIKEI_HITOTU_INDEX 1024
...
ANS_MAJIWARITIKEITOSOREIGAI majiwariAtariUnitTikeiToSoreigai(MESH_VERTEXKARI vertex1,
 ...
 // obb1.c = float3(0,0,0);
  ...
  if(ans.is_majiwari) {
	...
	//ans.housen = normalize(ans.housen + norm3);
  }
...
#define ATARI_WAZA 3
#define ATARI_TIKEI 0

// group �ɂ�max�C���X�^���X�̐���w�肷��E
// numthreads �ɂ�maxbonenum �̐���w�肷��E
[numthreads(16,8,1)]
void CalcCS(uint3 gtid: SV_GroupID, uint gid: SV_GroupIndex )
{

	uint struct_idx = ansBuf.IncrementCounter();// (gtid.x * 32 * 32 + gtid.y * 32 + gtid.z) * 128 + gid;// ansBuf.IncrementCounter();// //ansBuf.IncrementCounter();// 
uint k_count = kumi_count;
uint igai_dousi_k_count = igaidousi_count;
uint soreigai_idx = struct_idx/ KTROBO_MAX_TIKEI_HITOTU_INDEX;//gtid.x*32*32+gtid.y*32+gtid.z;//struct_idx / 128;//KTROBO_MAX_TIKEI_HITOTU_INDEX;
uint soreigai_index_idx = struct_idx % KTROBO_MAX_TIKEI_HITOTU_INDEX;//(struct_idx - soreigai_idx*128);//KTROBO_MAX_TIKEI_HITOTU_INDEX;
/*
int idxd;
idxd = ansBuf2.IncrementCounter();
if (idxd >= ans_count) return;
if (idxd >= soreigai_count) return;
ansBuf2[idxd].is_use = 1;
ansBuf2[idxd].atari_idx = 4567;// auttsBuf[idxd].atari_idx;
ansBuf2[idxd].atari_idx2 = auttsBuf[idxd].atari_idx2;

ansBuf2[idxd].obb_idx = 4567;// auinfoBuf[auttsBuf[idxd].atari_idx].atari_idx;// autts.obb_idx;
ansBuf2[idxd].obb_idx2 = idxd;// auinfoBuf[auttsBuf[idxd].atari_idx2].atari_idx;
ansBuf2[idxd].kouten_jyusin = auinfoBuf[idxd].jyusin;// float3(1, 1, 1);// at.pos.x, at.pos.y, at.pos.z);
ansBuf2[idxd].kouten_housen = float3(auinfoBuf[auttsBuf[idxd].atari_idx].r, auinfoBuf[auttsBuf[idxd].atari_idx2].r, auinfoBuf[idxd].r);// at.housen.x, at.housen.y, at.housen.z);
return;
...
if (soreigai_count > soreigai_idx) {
  ...
  int au2_indexs_count = au2.index_count;
    ...
    int inde1 = getIndex(au2_indexs_place);
    int inde2 = getIndex(au2_indexs_place +1);
    int inde3 = getIndex(au2_indexs_place +2);
	...
	float dtt = (au1.dt + au2.dt)/2;
	float obb1u0 = abs(dot(au1.v * dtt/6,obb1.u[0]));
	float obb1u1 = abs(dot(au1.v * dtt/6,obb1.u[1]));
	float obb1u2 = abs(dot(au1.v * dtt/6,obb1.u[2]));
	AtariUnitOBB obbdayo;
	obbdayo.c = obb1.c - au1.v * dtt/12;
	obbdayo.u[0] = obb1.u[0];
	obbdayo.u[1] = obb1.u[1];
	obbdayo.u[2] = obb1.u[2];
	obbdayo.e[0] = obb1.e[0];
	obbdayo.e[1] = obb1.e[1];
	obbdayo.e[2] = obb1.e[2];
	
	obbdayo.e[0] = obb1.e[0]*2.2f + obb1u0;
	obbdayo.e[1] = obb1.e[1]*2.2f + obb1u1;
	obbdayo.e[2] = obb1.e[2]*2.2f + obb1u2;


	if (au1.unit_type == ATARI_TIKEI) {
		obbdayo.e[0] = obb1.e[0]*1.05f + obb1u0;
		obbdayo.e[1] = obb1.e[1]*1.05f + obb1u1;
		obbdayo.e[2] = obb1.e[2]*1.05f + obb1u2;
	
	}
	
	if (au2.unit_type == ATARI_TIKEI) {
		obbdayo.e[0] = obb1.e[0]*1.05f + obb1u0;
		obbdayo.e[1] = obb1.e[1]*1.05f + obb1u1;
		obbdayo.e[2] = obb1.e[2]*1.05f + obb1u2;
		...
		if (au1.unit_type == ATARI_WAZA) {
		obbdayo.e[0] = obb1.e[0]*1.65f + obb1u0;
		obbdayo.e[1] = obb1.e[1]*1.65f + obb1u1;
		obbdayo.e[2] = obb1.e[2]*1.65f + obb1u2;
	
	}
	
	if (au2.unit_type == ATARI_WAZA) {
		obbdayo.e[0] = obb1.e[0]*1.65f + obb1u0;
		obbdayo.e[1] = obb1.e[1]*1.65f + obb1u1;
		obbdayo.e[2] = obb1.e[2]*1.65f + obb1u2;
