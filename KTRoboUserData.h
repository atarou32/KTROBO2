...
public:
	...
	public:
		...
		ShopParts::PartsListCategory getLegCategory();
		bool containsThisItem(int item_id,ItemWithCategory* c);
		void emptyItem(ShopParts::PartsListCategory category);
...
public:
	...
	bool sellItemInShop(int item_id, ItemWithCategory* item); // AsmBodyfileに使っているものであれば消させない

	//void loadItem(int item_id) {}; // parts の　mesh までロードする

	bool isThisItemUsedInAllAsmBody(int item_id, ItemWithCategory* i);
